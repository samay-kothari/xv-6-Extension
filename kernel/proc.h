...
struct proc {
  ...
  uint64 queueNumber;          // Queue number for the process
  uint64 queueEntry;           // Time of the entry in the queue
  uint64 curTicks;             // Number of time ticks lived in current process
  uint64 nextSwitch;           // Process have lived its life in queue and is now moving to queue with lower priority
  uint64 queueTime[5];         // Time spent in each queue
};
