...
procinit(void)
{
  ...
  #if SCHEDULER == 3
    for(int i = 0; i < 5 ; i++){
      queue_mlfq[i].numProcesses = 0;
      for(int k = 0; k < queue_mlfq[i].numProcesses ; k++){
        queue_mlfq[i].processes[k] = 0;
      }
    }
  #endif
}
...
scheduler(void)
{
  ...
  c->proc = 0;
    ...
    for(;;){
      ...
      // Avoid deadlock by ensuring that devices can interrupt.
      intr_on();
      struct proc *carryProc;
      // checking queues if they are aged
      for(int i = 1; i < 5; i++){
      ...
      for(int i = 0; i < 5 ; i++){
        if(queue_mlfq[i].numProcesses > 0){
          p = queue_mlfq[i].processes[0];
          queue_mlfq[i].processes[0] = 0;
          deleteFromQueue(i, 0);
          break;
        }
      }
      ...
      if(p->state == RUNNABLE){
        ...
        if(p->state == RUNNABLE){
          ...
          p->wtime = 0;
          p->curTicks = 0;
          if(p->nextSwitch && (p->queueNumber < 4)){
            p->queueNumber++;
          }
          addToQueue(p->queueNumber, p);
          p->nextSwitch = 0;
        }
      }
      release(&p->lock);
...
procdump(void)
{
  ...
  printf("\n");
    ...
    for(p = proc; p < &proc[NPROC]; p++){
      ...
      {
        p->wtime = p->etime - p->ctime - p->rtime;
      }
      printf("%d\t%d\t\t%s\t%d\t%d\t%d", p->pid, getDP(p->priority, p->rtime, p->stime), state, p->rtime, p->wtime, p->schedules);
      printf("\n");
    }
  #elif SCHEDULER == 3
